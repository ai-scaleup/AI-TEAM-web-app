...
export default function App() {
  ...
  const N8N_ENDPOINT =
    "https://n8n-c2lq.onrender.com/webhook/d601660e-0387-406d-b314-3db2b503708e/chat?action=sendMessage"
  ...
  const deleteChat = (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (remainingIds.length === 0) {
      ...
      localStorage.setItem("aladino-ai-chats", JSON.stringify(newChatState))
  ...
  const sendMessage = async () => {
    ...
    if (!currentChatIdForSend) {
      ...
      localStorage.setItem("aladino-ai-chats", JSON.stringify(updatedChatsState))
    } else {
  ...
  <div className="w-9 h-9 md:w-10 md:h-10 rounded-full shadow-lg shadow-sky-500/30 shrink-0 border-2 border-white dark:border-slate-900 overflow-hidden">
    <img 
      src={currentAgent.image || "https://www.ai-scaleup.com/wp-content/uploads/2025/02/Aladdin-AI-consultant.png"}
      alt={currentAgent.name}
      className="w-full h-full object-cover"
    />
  </div>
