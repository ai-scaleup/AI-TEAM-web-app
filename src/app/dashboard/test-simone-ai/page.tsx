...
export default function App() {
  ...
  const N8N_ENDPOINT =
    "https://n8n-c2lq.onrender.com/webhook/2c20e4ed-16d7-417a-a325-d20f7d81f08c/chat?action=sendMessage"
  ...
  const initNewChatForAgent = (agent: any, specificAgentId?: string) => {
    ...
    if (agent.name === "Simone AI") {
      messageText = `Ciao, sono Simone AI. Il tuo AI SEO Copywriter specializzato nella creazione di contenuti testuali di alta qualità che si massimizzino su Google. Come posso supportarti oggi?`
    }
  ...
  const confirmCreateFolder = () => {
    ...
    localStorage.setItem("simone-ai-folders", JSON.stringify(updatedFolders))
            ...
            <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 border border-sky-100 dark:border-white/5">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.setItem("simone-ai-use-memory", String(!useMemory))
                }}
  ...
  <div className="w-9 h-9 md:w-10 md:h-10 rounded-full shadow-lg shadow-sky-500/30 shrink-0 border-2 border-white dark:border-slate-900 overflow-hidden">
    <img 
      src={currentAgent.image || "https://www.ai-scaleup.com/wp-content/uploads/2025/02/Simone-AI-seo-copy.png"}
      alt={currentAgent.name}
      className="w-full h-full object-cover"
    />
  </div>
