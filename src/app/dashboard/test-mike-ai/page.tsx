...
export default function App() {
  // --- STATE ---
  const [activeAgentId, setActiveAgentId] = useState<string>("mike-ai")
  ...
  const N8N_ENDPOINT =
    "https://n8n-c2lq.onrender.com/webhook/b4b91b86-1adc-4e28-8fad-37ce72789cc3/chat?action=sendMessage"
  ...
  useEffect(() => {
    ...
    const savedFolders = localStorage.getItem("mike-ai-folders")
    if (savedFolders) {
      try {
  ...
  const initNewChatForAgent = (agent: any, specificAgentId?: string) => {
    ...
    if (agent.name === "Mike AI") {
      messageText = `Ciao! Sono Mike AI. Il tuo stratega di marketing. Definisco funnel e strategie integrate per scalare il business. Come posso aiutarti?`
    }
  ...
  const confirmCreateFolder = () => {
    ...
    localStorage.setItem("mike-ai-folders", JSON.stringify(updatedFolders))
