...
export default function App() {
  // --- STATE ---
  const [activeAgentId, setActiveAgentId] = useState<string>("niko-ai")
  const currentAgent = AGENTS_DB[activeAgentId] || AGENTS_DB["niko-ai"]
  ...
  const N8N_ENDPOINT =
    "https://n8n-c2lq.onrender.com/webhook/65dd9b3d-a55c-41bc-943e-7ba9e38646b6/chat?action=sendMessage"
  ...
  useEffect(() => {
    ...
    const savedChats = localStorage.getItem("niko-ai-chats")
    if (savedChats) {
      try {
    ...
    const savedFolders = localStorage.getItem("niko-ai-folders")
    if (savedFolders) {
      try {
  ...
  const initNewChatForAgent = (agent: any, specificAgentId?: string) => {
    ...
    if (agent.name === "Niko AI") {
      messageText = `Ciao, sono Niko AI. Il tuo SEO Strategist virtuale, progettato per sviluppare una strategia SEO a lungo termine e migliorare la tua presenza online. Come posso supportarti oggi?`
    }
    ...
    setChats((prev) => {
      ...
      localStorage.setItem("niko-ai-chats", JSON.stringify(newChats))
      return newChats
    })
  }
  ...
  const confirmCreateFolder = () => {
    ...
    localStorage.setItem("niko-ai-folders", JSON.stringify(updatedFolders))
  ...
  const deleteChat = (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (remainingIds.length === 0) {
      ...
      localStorage.setItem("niko-ai-chats", JSON.stringify(newChatState))
  ...
  const sendMessage = async () => {
    ...
    if (!currentChatIdForSend) {
      ...
      localStorage.setItem("niko-ai-chats", JSON.stringify(updatedChatsState))
    } else {
            ...
            <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 border border-sky-100 dark:border-white/5">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.setItem("niko-ai-use-memory", String(!useMemory))
                }}
  ...
  <div className="w-9 h-9 md:w-10 md:h-10 rounded-full shadow-lg shadow-sky-500/30 shrink-0 border-2 border-white dark:border-slate-900 overflow-hidden">
    <img 
      src={currentAgent.image || "https://www.ai-scaleup.com/wp-content/uploads/2025/02/Niko-AI.png"}
      alt={currentAgent.name}
      className="w-full h-full object-cover"
    />
  </div>
