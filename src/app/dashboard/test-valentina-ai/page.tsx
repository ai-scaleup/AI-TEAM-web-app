...
export default function App() {
  // --- STATE ---
  const [activeAgentId, setActiveAgentId] = useState<string>("valentina-ai")
  const currentAgent = AGENTS_DB[activeAgentId] || AGENTS_DB["valentina-ai"]
  ...
  const N8N_ENDPOINT =
    "https://n8n-c2lq.onrender.com/webhook/4429872c-62ca-4848-9caa-f1c6759b6737/chat?action=sendMessage"
  ...
  useEffect(() => {
    ...
    const savedFolders = localStorage.getItem("valentina-ai-folders")
    if (savedFolders) {
      try {
  ...
  const initNewChatForAgent = (agent: any, specificAgentId?: string) => {
    ...
    if (agent.name === "Valentina AI") {
      messageText = `Ciao, sono Valentina AI. la tua esperta di SEO, specializzata nell'ottimizzazione dei contenuti già pubblicati e nel posizionamento sui motori di ricerca. Come posso aiutarti oggi?`
    }
    ...
    setChats((prev) => {
      ...
      localStorage.setItem("valentina-ai-chats", JSON.stringify(newChats))
      return newChats
    })
  }
  ...
  const confirmCreateFolder = () => {
    ...
    localStorage.setItem("valentina-ai-folders", JSON.stringify(updatedFolders))
  ...
  const deleteChat = (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (remainingIds.length === 0) {
      ...
      localStorage.setItem("valentina-ai-chats", JSON.stringify(newChatState))
  ...
  const sendMessage = async () => {
    ...
    if (!currentChatIdForSend) {
      ...
      localStorage.setItem("valentina-ai-chats", JSON.stringify(updatedChatsState))
    } else {
            ...
            <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 border border-sky-100 dark:border-white/5">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.setItem("valentina-ai-use-memory", String(!useMemory))
                }}
  ...
  <div className="w-9 h-9 md:w-10 md:h-10 rounded-full shadow-lg shadow-sky-500/30 shrink-0 border-2 border-white dark:border-slate-900 overflow-hidden">
    <img 
      src={currentAgent.image || "https://www.ai-scaleup.com/wp-content/uploads/2025/03/Valentina-AI-AI-SEO-optimizer.png"}
      alt={currentAgent.name}
      className="w-full h-full object-cover"
    />
  </div>
